...
class Solver():
    ...
    def solve_fully_consistent(self, lambda_reg=1e-1, epsillon=1e-3, max_iter=20000, reg_type="none"):
        """
        ...
        for idx in range(0, self._num_links):
            ...
            Pinertia = self._construct_pseudo_inertia_matrix(phi_idx.value)
            J = cp.Parameter(Pinertia.shape, value=Pinertia, symmetric=True)
            ...
            self._constraints.append(com_constraint_param >= cp.Constant(0))
            ...
            self._constraints.append(cp.trace(Pinertia @ Q_ellipsoid) >= cp.Constant(0))

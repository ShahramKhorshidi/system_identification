...
class PlotClass():
    ...
    def plot_h(self, phi_ident, title):
        ...
        for i in range(num_links):
            ...
            h_x_actual.append(self.phi_nominal[idx + 1])
            h_y_actual.append(self.phi_nominal[idx + 2])
            h_z_actual.append(self.phi_nominal[idx + 3])
    ...
    def plot_inertia(self, phi_ident, title):
        ...
        for i in range(num_links):
            idx = i * num_params_per_link
            I_xx_actual.append(self.phi_nominal[idx + 4])
            I_xy_actual.append(self.phi_nominal[idx + 5])
            I_yy_actual.append(self.phi_nominal[idx + 6])
            I_xz_actual.append(self.phi_nominal[idx + 7])
            I_yz_actual.append(self.phi_nominal[idx + 8])
            I_zz_actual.append(self.phi_nominal[idx + 9])
    ...
    def proj_torques_null(self, q, dq, ddq, cnt, torque, b_v, b_c, phi, robot_dyn):
        tau_nn_proj = []
        # For each data ponit we calculate the rgeressor and torque vector, and stack them
        for i in range(q.shape[1]):
            robot_dyn.update_fk(q[:, i], dq[:, i], ddq[:, i])
            Y = robot_dyn.get_regressor_matrix(q[:, i], dq[:, i], ddq[:, i])
            P = robot_dyn.get_null_space_proj(cnt[:, i])
            tau_meas = P @ robot_dyn.S.T @ torque[:, i]
            tau_nn_proj.append(tau_meas[6:])
        
        tau_nn_proj = np.vstack(tau_nn_proj)
        return tau_nn_proj
    
    def plot_proj_torques(self, q, dq, ddq, cnt, torque, b_v, b_c, phi, robot_dyn, title):
        predicted = []
        measured = []
        # For each data ponit we calculate the rgeressor and torque vector, and stack them
        for i in range(q.shape[1]):
            ...
            P = robot_dyn.get_null_space_proj(cnt[:, i])
            tau_pred = P @ (Y @ phi 
                            - robot_dyn.S.T @ (np.diag(b_v) @ dq[robot_dyn.base_dof:, i] 
                            + np.diag(b_c) @ np.sign(dq[robot_dyn.base_dof:, i])))
            tau_meas = P @ robot_dyn.S.T @ torque[:, i]
    ...
    def plot_solo_torques(self, q, dq, ddq, cnt, torque, b_v, b_c, phi, robot_dyn, title, force):
        predicted = []
        measured = []
        # For each data ponit we calculate the rgeressor and torque vector, and stack them
        for i in range(q.shape[1]):
            ...
            tau_pred = (Y @ phi 
                        - robot_dyn.S.T @ (np.diag(b_v) @ dq[robot_dyn.base_dof:, i] 
                        + np.diag(b_c) @ np.sign(dq[robot_dyn.base_dof:, i])))
            F = robot_dyn.get_cnt_force(force[:, i], cnt[:, i])
            tau_pred -= F
            predicted.append(tau_pred[6:])

...
if __name__ == "__main__":
    ...
    load_path = os.path.join(parentDirPath, "data/spot/")
    motion_name = "train"
    q = np.loadtxt(load_path+motion_name+"_robot_q.dat", delimiter='\t', dtype=np.float32)
    dq = np.loadtxt(load_path+motion_name+"_robot_dq.dat", delimiter='\t', dtype=np.float32)
    ddq = np.loadtxt(load_path+motion_name+"_robot_ddq.dat", delimiter='\t', dtype=np.float32)
    tau = np.loadtxt(load_path+motion_name+"_robot_tau_proj.dat", delimiter='\t', dtype=np.float32)[6:, :]
    cnt = np.loadtxt(load_path+motion_name+"_robot_contact.dat", delimiter='\t', dtype=np.float32)
    ...
    N_s = 2500
    for i in range(7):
        X_T[(i*N_s)+1:(i+1)*N_s, 0:2] = X_T[(i*N_s)+1:(i+1)*N_s, 0:2] - X_T[(i*N_s):(i+1)*N_s-1, 0:2]
        X_T[i*N_s, 0:2] = 0  
    ...
    size = 9000 # Number of samples used for training

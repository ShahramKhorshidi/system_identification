...
def get_projected_y_tau(q, dq, ddq, torque, cnt, quad_dyn):
    ...
    for i in range(q.shape[1]):
        y, tau = quad_dyn.get_proj_regressor_torque(q[:, i], dq[:, i], ddq[:, i], torque[:, i], cnt[:, i])
        Y.append(y)
        Tau.append(tau)
    return Y, Tau
...
def get_projected_friction_regressors(q, dq, ddq, cnt, quad_dyn):
    ...
    # For each data ponit we calculate the rgeressor and torque vector, and stack them
    for i in range(q.shape[1]):
        b_v, b_c = quad_dyn.get_proj_friction_regressors(q[:, i], dq[:, i], ddq[:, i], cnt[:, i])
        B_v.append(b_v)
        B_c.append(b_c)
    return B_v, B_c
...
def main():
    ...
    np.savetxt(path+"/data/go1/"+motion_name+"_phi_prior.dat", phi_nominal, delimiter='\t')
    ...
    B_v = np.vstack(B_v)
    B_c = np.vstack(B_c)
    solver = LMISolver(Y_proj, Tau, num_of_links, phi_nominal, total_mass, bounding_ellipsoids, B_v=B_v, B_c=B_c)
    
    phi_proj_llsq = solver.solve_llsq_svd()
    np.savetxt(path+"/data/go1/"+motion_name+"_phi_proj_llsq.dat", phi_proj_llsq, delimiter='\t')
    
    phi_proj_lmi = solver.solve_fully_consistent()

...
class SystemParameters:
    ...
    def __init__(self):
        ...
        self.file_names = np.array([
                                        "spot_crawl_speed_slow_height_high_moved_forward", "spot_crawl_speed_slow_height_high_turn_around",
                                        "spot_pose", "spot_squat",
                                        "spot_walk_speed_slow_height_high_moved_forward", "spot_walk_speed_slow_height_high_turn_around",
                                        ])
        ...
        self.sys_idnt.compute_bounding_ellipsoids()
    ...
    def solve(self, Y, Tau, trajectory_name, filter_name, trajectory_names, do_filter, do_odom):
        num_of_links = self.sys_idnt.get_num_links()
        phi_prior = self.sys_idnt.get_phi_prior()
        total_mass = self.sys_idnt.get_robot_mass()
...
if __name__ == "__main__":
    ...
    system_parameters.compute_inertial_parameters_for_some_trajectories_together(system_parameters.file_names, "good_trajectories", do_filter=False, do_odom=True)
    system_parameters.compute_inertial_parameters_for_some_trajectories_together(system_parameters.file_names, "good_trajectories", do_filter=False, do_odom=False)
    system_parameters.compute_inertial_parameters_for_each_trajectory_independent(system_parameters.file_names, do_filter=False, do_odom=True)
    system_parameters.compute_inertial_parameters_for_each_trajectory_independent(system_parameters.file_names, do_filter=False, do_odom=False)
